...
def inferenceByEnumeration(bayesNet, queryVariables, evidenceDict):
    ...
    curFactList = bayesNet.getAllCPTsWithEvidence(evidenceDict)

    # join all factors by variable
    for joinVariable in bayesNet.variablesSet():
        curFactList, joinedFactor = joinFactorsByVariable(curFactList, joinVariable)
        curFactList.append(joinedFactor)

    # curFactList should contain the connected components of the graph now as factors, must join the connected components
    fullJoint = joinFactors(curFactList)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        for var in eliminationOrder:
            curFactList, joinedFactor = joinFactorsByVariable(curFactList, var)
            
            length = len(joinedFactor.unconditionedVariables())
            if  length > 1:
                curFactList.append(eliminate(joinedFactor, var))
        
        joined = joinFactors(curFactList)
