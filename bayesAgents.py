...
def fillObsCPT(bayesNet, gameState):
    ...
    def checkPos(wallp, housep):
        firstCheck = abs(wallp[0] - housep[0])
        secondCheck = abs(wallp[1] - housep[1])
        return (firstCheck <= 1) and (secondCheck <= 1)
    ...
    for house in gameState.getPossibleHouses():
        for walls in gameState.getHouseWalls(house):
            ...
            if checkPos(walls, bottomLeftPos):
                pos = BOTTOM_LEFT_VAL
            elif checkPos(walls, bottomRightPos):
                pos = BOTTOM_RIGHT_VAL
            elif checkPos(walls, topLeftPos):
                pos = TOP_LEFT_VAL
            elif checkPos(walls, topRightPos):
                pos = TOP_RIGHT_VAL
            else:
                pos = None
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    pos = ['foodHouse', 'ghostHouse']
    factor = inference.inferenceByVariableElimination(bayesNet, pos, evidence, eliminationOrder)
    p = 0.0
    assignment = dict()

    assignments = factor.getAllPossibleAssignmentDicts()
    for a in assignments:
        currentProb = factor.getProbability(a)
